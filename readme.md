(chrome extention - json viewer, cors allow extention, react dev tool)

Config-driven UI , lifting state up, props drill
...
# pros drilling - data or props pass from parent to child, child to it's child & it's child & etc (this is practically an issue)
# react context - to resolved props drilling problem context are used in react
    - context - this is basily a global variable or data which can easily access from anywhere, to crate this global data react provide a 'createContext' & we can use this using "useContext" hook;
    - In  class based component we can use 'Consumer' e.g. '<UserContext.Consumer>callback fun - inside this we can get context data</UserContext.Consumer>'
    - you can use 'context' anywhere in application or modify the info of context e.g authentication 
    - use Provider & wrap data in it 
...
# Redux
    ...
    for collecting (subscribing data) a data from store we use selector & it's a basically hook 'useSelector' from react redux